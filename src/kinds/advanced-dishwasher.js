...
// ── a circular PROGRESS RING face — a donut whose coloured arc sweeps clockwise from the top
// to `--adw-prog`% (an ordinal level, e.g. water hardness). Reads as a level meter, not a
// twiddleable knob. The inner disc punches the hole; the value is the tile's label below. ──
const adwRingFace = (root, size) => `
      ${root} {
        ${size}
        border-radius: 50% !important;
        position: relative;
        background: conic-gradient(from 0deg, rgb(var(--adw-rgb, 33, 150, 243)) calc(var(--adw-prog, 0) * 1%), rgba(120, 130, 140, 0.18) 0) !important;
      ...
      ${root}::before {
        ...
        inset: 7px;
        border-radius: 50%;
        background: linear-gradient(180deg, #f4f6f8 0%, #e6eaee 100%) !important;
        box-shadow: inset 0 1px 2px rgba(0, 0, 0, 0.12), 0 1px 1px rgba(255, 255, 255, 0.7);
      }
      ${root}::after {
        content: '';
        position: absolute;
        inset: 3px;
        border-radius: 50%;
        transform: rotate(calc(var(--adw-prog, 0) * 3.6deg));
        background:
          radial-gradient(circle 3px at 50% 3px, #ffffff 0 2.5px, rgba(255, 255, 255, 0.9) 2.5px 3px, transparent 3.5px);
        z-index: 2;
      }`;
...
// ── an ordinal PROGRESS-RING tile (e.g. water hardness): the ring arc = the option's index
// over the scale; label = the option name; tap → the pop-over at `hash`. ────────────────────
const adwRing = (entity, conn, opts, label, rgb, hash) => ({
  type: "custom:mushroom-template-card",
  entity,
  primary: label,
  ...
  tap_action: { action: "navigate", navigation_path: hash },
  card_mod: { style: {
    "ha-tile-icon$": adwRingFace(".container", "width: 48px !important; height: 48px !important;"),
    "mushroom-shape-icon$": adwRingFace(".shape", "--icon-size: 48px !important; width: var(--icon-size) !important; height: var(--icon-size) !important;"),
    ".": `
      ha-tile-icon { --tile-icon-size: 48px; width: 48px; height: 48px; }
      mushroom-shape-icon { --icon-size: 48px; }
      ha-state-icon, ha-icon { display: none !important; }
      ...
      ha-card {
        ...
        {% set v = states('${entity}') %}
        {% set opts = [${opts.map((o) => `'${o}'`).join(", ")}] %}
        {% set idx = opts.index(v) if v in opts else 0 %}
        --adw-prog: {{ (idx / (opts | length - 1) * 100) | round(0) }};
        --adw-rgb: ${rgb};
        ${AW_FLAT}
        --card-primary-font-size: 0.8rem;
        --card-primary-font-weight: 600;
...
const adwMake = (c) => {
  ...
  const hardDial = adwRing(hard, conn, ADW_HARD, hardLabel, ADW_HARD_RGB, hardHash);

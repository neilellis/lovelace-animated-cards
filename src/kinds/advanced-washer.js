...
// The white appliance fascia every card in the stack wears. Mushroom reads its text colours
// from these vars, so dark-on-white holds in any theme.
const AW_FACE = `
...
// section label, printed on the fascia like the machine's silk-screened text
const awTitle = (subtitle) => ({
  type: "custom:mushroom-title-card",
  subtitle,
  card_mod: { style: { ".": `
      .header { --title-subtitle-color: #8a949c; letter-spacing: 1.5px; text-transform: uppercase; font-size: 0.8rem; }` } },
});
...
const awMake = (c, parts) => {
  ...
  if (parts.settings) {
    cards.push(awTitle("Programme settings"));
  ...
  if (parts.options) {
    cards.push(awTitle("Extra options"));
    cards.push({ type: "horizontal-stack", cards: [
